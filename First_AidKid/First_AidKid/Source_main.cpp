...
int min_doctors = 2, max_doctors = 2,
...
void main() {
	...
	for(num_doctors = min_doctors; num_doctors <= max_doctors; num_doctors++){
		...
		// изменение параметра "вероятность определить диагноз"
		for(int probToDetermDiagnStep = 0; probToDetermDiagnStep < 10; probToDetermDiagnStep++){
			for(int itemDiagn = 0; itemDiagn <= num_doctors; itemDiagn++){

				// изменение параметра "вероятность повторного приёма"
				for(int probOfReTakingStep = 0; probOfReTakingStep < 4; probOfReTakingStep++){
					for(int itemReTacking = 0; itemReTacking <= num_doctors; itemReTacking++){

						// изменение параметра "вероятность отправить на лечение к другому врачу"
						for(int probSendToAnotherStep = 0; probSendToAnotherStep < 6; probSendToAnotherStep++){
							for(int itemToAnother = 0; itemToAnother <= num_doctors; itemToAnother++){

								clock_t begin;
								clock_t end;
								double time_spent;

								// для каждого из вариантов сочетания
								for(int Cnk = 2; Cnk <= num_doctors; Cnk++){
									...
									connections.clear();
									getCnk(num_doctors, Cnk);

									// simulation cicle  connections.size() 0
									for(int i = 0; i < connections.size(); i++){
										for(int j = 1; j < connections[i].size(); j++) {	// one simulation cicle
								...
								// прибавляем шаг к параметру "вероятность отправить на лечение к другому врачу"
								doctors[itemReTacking].probSendToAnother += 0.05;
							}
						}
						// прибавляем шаг к параметру "вероятность повторного диагноза"
						doctors[itemReTacking].probOfReTaking += 0.05;
					}
				}
				// прибавляем шаг к параметру "вероятность определить диагноз"
				doctors[itemDiagn].probToDetermDiagn += 0.05;
			}
		}
	}	// изменение кол-ва врачей min -> max | step: +1
	fclose(infile);
	fclose(outfile);
